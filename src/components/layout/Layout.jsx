...
const Layout = ({ children }) => {
  ...
  const getLiveStreaming = async () => {
    try {
      ...
      dispatch(
        checkLiveNewsData({
          data: { liveNewsDataFound: data.data?.length > 0 ? true : false, isLiveNewsApiCallOnce: true }
        })
      )
  ...
  const getBreakingNewsApi = async () => {
    try {
      ...
      dispatch(
        checkBreakingNewsData({
          data: { breakingNewsDataFound: data.data?.length > 0 ? true : false, isBreakingNewsApiCallOnce: true }
        })
      )
      return data.data
    } catch (error) {
