...
import { loadNews, newsUpdateLanguage } from '../../store/reducers/newsReducer'
                  ...
                  <div className={`cn-main-div catSubCatWrapper flex-display`}>
                    {categories.map((element, index) => (
                      <div key={element.id} className='text-center'>
                        ...
                        categoiresOnOff &&
                        categoiresOnOff.subcategory_mode === '1' ? (
                          <span
                            className={`catNav-links  ${
                              subCatDrop && currentCategory && currentCategory.id === element.id ? 'activeSubDrop' : ''
                            }`}
                          ...
                          subCatDrop && currentCategory && currentCategory.id === element.id ? (
                            <>
                              <div className='subCatDropdown'>
                                <div className='row' onMouseLeave={() => setSubCatDrop(false)}>
                                  <div className='col-lg-3'>
                                    <div className='subCatNamesWrapper'>
                                      <div onClick={() => setSubCatSlug('')}>
                                        <span className={subCatSlug === '' ? 'subNavActive' : ''}>All</span>
                                        {subCatSlug === '' ? <FaChevronRight /> : null}
                                      </div>
                                      {currentCategory.sub_categories?.map(e => {
                                        return (
                                          <div onClick={() => setSubCatSlug(e?.slug)} key={e.slug}>
